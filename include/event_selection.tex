...
Additional kinematic selection criteria are used for the resonance and
angular search distributions separately to optimise the search
potential and ensure good tracking efficiency.


The following cuts are also applied to the quark-gluon tagged resonance searches:\todo{This order might change.}
\begin{itemize}
\item $|\ystar| < 0.6$
\item \todo{redo as cut in \yB.} $| \eta | < 2.1$
\item $\mjj > 1100~\GeV$
\end{itemize}

Given its different decay topology, for the search of String signal slightly different cuts are applied:\todo{This has not yet been decided. A smaller $\ystar$ cut might be better.}
\begin{itemize}
\item $|\ystar| < 1.2$
\item $\mjj > 1717~\GeV$
\end{itemize}
